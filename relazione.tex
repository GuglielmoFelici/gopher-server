...
-h
    Mostra la sintassi.
-l FILE
    Logga i trasferimenti sul file FILE. Di default il logging avviene sul file "logfile", nella stessa directory dell'eseguibile.
-m 
    Utilizza un processo per ogni client. Disabilitato di default.
-p PORT
    Metti il server in ascolto sulla porta PORT. 7070 di default.
-s
    (silent) - Non loggare in console/syslog. Disabilitato di default.
...
Da qui la scelta di utilizzare diverse configurazioni globali.\\
...
{main loop} del server; tuttavia, nel caso in cui si rileggano le configurazioni mentre il server è in esecuzione,
verranno ricaricate solo le impostazioni port, multiprocess e silent.\\
